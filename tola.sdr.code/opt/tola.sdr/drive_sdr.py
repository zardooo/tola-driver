...
def servant4(protocol, ip, tuner_sema, reg_sema, tune, logger, port = 65421):
    ...
    while True:
        ...
        if ipv4_threads == []:             #first running
            pass
        elif ipv4_threads[-1].isAlive():  #if connection exist, shut down new
            sock.close()
...
def servant6(protocol, ip, tuner_sema, reg_sema, tune, tola, logger, port = 65420):
    ...
    while True:
        ...
        msg = sock.recv(1024)
        if msg == b'hello':             #receive hello to establish connection
...
def tcplink(sock, addr, tuner_sema, reg_sema, tola, logger):
    ...
    while True:
        ...
        if msg == b'shutdown':
            ...
            date_msg = sock.recv(1024)    #receive run time
            date_msg = date_msg.decode('utf-8')
            logger.write(date_msg)
            logger.write('\n')

            scheduler = BackgroundScheduler()   #set run time
